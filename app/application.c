...
void button_event_handler(bc_button_t *self, bc_button_event_t event, void *event_param)
{
    ...
    if(event == BC_BUTTON_EVENT_PRESS)
    {
        ...
        uint8_t val = 63; // ((uint8_t)temperatures[0]) - 20;

        uint8_t send_array[51];
        memset(send_array, 0x00, sizeof(send_array));

        uint32_t bit = 0;
        uint32_t i;
        for(i = 0; i < 64; i++)
        {
            val = ((uint8_t)temperatures[i]) - 20;
            uint32_t b = bit / 8;
            uint32_t offset = bit % 8;
            if(offset > 2)
            {
                send_array[b] |= val << offset;
                send_array[b+1] |= val >> (8-offset);
            }
            else
            {
                send_array[b] |= val << offset;
        ...
        bc_cmwx1zzabz_send_message(&lora, send_array, sizeof(send_array) );
...
void application_task()
{
    ...
    uint32_t color = 128;
    bc_gfx_draw_string(&gfx, 3, -2, display_message, color<<16);
